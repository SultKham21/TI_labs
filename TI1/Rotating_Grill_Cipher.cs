...
namespace TI
{
    public static class RotatingGrillCipher
    {
        ...
        public static void CallRotateGrill()
        {
            ...
            while (flag)
            {
                Console.Write("Размер решетки: ");
                var scanner = Console.ReadLine();
                n = int.Parse(scanner ?? string.Empty);

                if (n % 2 != 0 || n * n < text.Length)
                {
                    Console.WriteLine("Неверные данные. Повторите: ");
                ...
                else
                {
                    ...
                    Console.WriteLine("Шифр: " + encryption);
                    Console.WriteLine("Расшифровка: " + RotatingCipherDecrypt(encryption, n, lattice));
                }
            }
        }
        ...
        private static int[,] LatticeInput(int n)
        {
            ...
            for (int i = 0; i < (n / 2) * (n / 2); i++)
            {
                ...
                while (flag)
                {
                    Console.WriteLine("Координаты отверстия для " + (i + 1) + ":");
                    Console.Write("x: ");
